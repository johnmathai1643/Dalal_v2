var WebsocketClass = function(socket_url){
    ...
    this.channel_subscribe = function(channel_nm){
    	  if(establish_conn)
            { channel = dispatcher.subscribe(channel_nm);
              channel_link = true;
              binder = true;
              console.log("Channel established :"+channel_link + " " + channel_nm);
	        }
	      else
	...
	this.channel_binder = function(evt,callback_fn){
		if(channel_link)
        { 
          channel.bind(evt, function(data){ callback_fn(data); console.log("data received :"+data);console.log(channel);});
          // channel.unbind(evt);
        }
        else
